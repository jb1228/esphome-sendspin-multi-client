#include "sendspin_mc_hub.h"

#ifdef USE_ESP32

#include "esphome/components/network/util.h"
#ifdef USE_WIFI
#include "esphome/components/wifi/wifi_component.h"
#endif

#include "esphome/core/application.h"
#include "esphome/core/helpers.h"
#include "esphome/core/log.h"
#include "esphome/core/version.h"

#include <esp_log.h>
#ifdef USE_MDNS_SUPPORTS_ENABLE_DISABLE
#include <mdns.h>
#endif

namespace esphome::sendspin_mc {

static const char *const TAG = "sendspin_mc.hub";

#ifdef USE_SENDSPIN_MC_ARTWORK
// Indexed by the library enums, which start at zero and are contiguous.
static const char *const IMAGE_SOURCE_NAMES[] = {"ALBUM", "ARTIST", "NONE"};
static const char *const IMAGE_FORMAT_NAMES[] = {"JPEG", "PNG", "BMP"};
#endif

void SendspinMcHub::setup() {
  auto config = this->build_client_config_();
  this->client_ = std::make_unique<sendspin::SendspinClient>(std::move(config));

  // Set up persistence (preferences must be initialized before providers are added to the client)
  this->last_played_server_pref_ =
      global_preferences->make_preference<LastPlayedServerPref>(fnv1a_hash("sendspin_mc_last_played_" + this->client_id_));
#ifdef USE_SENDSPIN_MC_PLAYER
  if (this->player_support_) {
    this->static_delay_pref_ = global_preferences->make_preference<StaticDelayPref>(fnv1a_hash("sendspin_mc_static_delay_" + this->client_id_));
  }
#endif

  // Wire providers and client listener
  this->client_->set_listener(this);
  this->client_->set_network_provider(this);
  this->client_->set_persistence_provider(this);

#ifdef USE_SENDSPIN_MC_ARTWORK
  if (this->artwork_support_) {
    this->artwork_role_ = &this->client_->add_artwork(this->artwork_config_);
    this->artwork_role_->set_listener(this);
  }
#endif

#ifdef USE_SENDSPIN_MC_CONTROLLER
  if (this->controller_support_) {
    this->controller_role_ = &this->client_->add_controller();
    this->controller_role_->set_listener(this);
  }
#endif

#ifdef USE_SENDSPIN_MC_METADATA
  if (this->metadata_support_) {
    this->metadata_role_ = &this->client_->add_metadata();
    this->metadata_role_->set_listener(this);
  }
#endif

#ifdef USE_SENDSPIN_MC_PLAYER
  if (this->player_support_) {
    this->client_->add_player(this->player_config_).set_listener(this->player_listener_);
  }
#endif

}

void SendspinMcHub::loop() {
  if (this->enabled_.has_value() && this->enabled_.value() != this->client_->is_started() &&
      !this->status_has_error()) {
    if (!this->enabled_.value()) {
      this->client_->stop();
    } else if (!this->client_->start()) {
      this->status_set_error(LOG_STR("Failed to start Sendspin client"));
    }
  }
  this->client_->loop();

#ifdef USE_MDNS_SUPPORTS_ENABLE_DISABLE
  this->update_mdns_service_();
#endif
}

void SendspinMcHub::dump_config() {
  ESP_LOGCONFIG(TAG,
                "Sendspin MC Hub:\n"
                "  Client ID: %s\n"
                "  Client Name: %s\n"
                "  Server Port: %u\n"
                "  Control Port: %u\n"
                "  Manufacturer: %s\n"
                "  Model: %s\n"
                "  Firmware version: %s\n"
                "  Task stack in PSRAM: %s",
                this->client_id_.c_str(), this->client_name_.c_str(), this->server_port_, this->control_port_, this->manufacturer_, this->get_product_name_(),
                this->firmware_version_, YESNO(this->task_stack_in_psram_));

#ifdef USE_SENDSPIN_MC_ARTWORK
  // Slot indices come from the order the image platform entries were declared, so the log is the
  // only place the mapping from a slot to the artwork it asked for can be read back.
  uint8_t slot = 0;
  for (const auto &preference : this->artwork_config_.preferred_formats) {
    ESP_LOGCONFIG(TAG, "  Artwork slot %u: %s as %s, %ux%u, display offset %" PRId32 " ms", slot++,
                  IMAGE_SOURCE_NAMES[static_cast<uint8_t>(preference.source)],
                  IMAGE_FORMAT_NAMES[static_cast<uint8_t>(preference.format)], preference.width, preference.height,
                  preference.display_offset_ms);
  }
#endif
}

// THREAD CONTEXT: Main loop (invoked from Sendspin components)
void SendspinMcHub::set_enabled(bool enabled) {
  if (this->status_has_error()) {
    ESP_LOGE(TAG, "Cannot %s: Sendspin failed to start, reboot to retry",
             enabled ? LOG_STR_LITERAL("enable") : LOG_STR_LITERAL("disable"));
    return;
  }
  this->enabled_ = enabled;
}

#ifdef USE_MDNS_SUPPORTS_ENABLE_DISABLE
// THREAD CONTEXT: Main loop
void SendspinMcHub::update_mdns_service_() {
  // Synced from loop() because mdns sets up after this hub and only builds its service list then.
  if (!this->mdns_->is_ready()) {
    return;
  }
  bool advertise = this->client_->is_started();
  if (advertise == this->mdns_advertised_) {
    return;
  }
  // One attempt per change
  this->mdns_advertised_ = advertise;
  // TEMPORARY DEVIATION: Native mDNS only addresses services by type/protocol.
  // Name both operations so this hub cannot remove another client's service; see DEVIATIONS.md.
  mdns_txt_item_t txt_records[] = {{"path", "/sendspin"}};
  esp_err_t err = advertise
                      ? mdns_service_add(this->client_name_.c_str(), "_sendspin", "_tcp", this->server_port_,
                                         txt_records, 1)
                      : mdns_service_remove_for_host(this->client_name_.c_str(), "_sendspin", "_tcp", nullptr);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to %s mDNS service for %s: %s", advertise ? LOG_STR_LITERAL("enable") : LOG_STR_LITERAL("disable"),
             this->client_name_.c_str(), esp_err_to_name(err));
  }
}
#endif

// --- Delegating methods ---

// THREAD CONTEXT: Main loop (invoked from Sendspin components)
void SendspinMcHub::connect_to_server(const std::string &url) {
  if (this->is_client_running()) {
    this->client_->connect_to(url);
  }
}

// THREAD CONTEXT: Main loop (invoked from Sendspin components)
void SendspinMcHub::disconnect_from_server(sendspin::SendspinGoodbyeReason reason) {
  if (this->is_client_running()) {
    this->client_->disconnect(reason);
  }
}

// THREAD CONTEXT: Main loop (invoked from Sendspin components)
void SendspinMcHub::update_state(sendspin::SendspinClientState state) {
  if (this->is_client_running()) {
    this->client_->update_state(state);
  }
}

const char *SendspinMcHub::get_product_name_() const {
  return this->model_ != nullptr ? this->model_ : App.get_name().c_str();
}

sendspin::SendspinClientConfig SendspinMcHub::build_client_config_() {
  sendspin::SendspinClientConfig config;

  config.client_id = this->client_id_;
  config.name = this->client_name_;
  config.product_name = this->get_product_name_();
  config.manufacturer = this->manufacturer_;
  config.software_version = this->firmware_version_;
  config.server_port = this->server_port_;
  config.httpd_ctrl_port = this->control_port_;
  config.httpd_psram_stack = this->task_stack_in_psram_;

  return config;
}

// --- SendspinClientListener overrides ---
// THREAD CONTEXT: Main loop (fired from client_->loop())

void SendspinMcHub::on_group_update(const sendspin::GroupUpdateObject &group) {
  this->group_update_callbacks_.call(group);
}

void SendspinMcHub::on_request_high_performance() {
#ifdef USE_WIFI
  if (wifi::global_wifi_component != nullptr) {
    wifi::global_wifi_component->request_high_performance();
    wifi::global_wifi_component->request_roaming_suppression();
  }
#endif
}

void SendspinMcHub::on_release_high_performance() {
#ifdef USE_WIFI
  if (wifi::global_wifi_component != nullptr) {
    wifi::global_wifi_component->release_high_performance();
    wifi::global_wifi_component->release_roaming_suppression();
  }
#endif
}

// --- SendspinNetworkProvider override ---

// THREAD CONTEXT: Main loop (polled by client_->loop())
bool SendspinMcHub::is_network_ready() { return network::is_connected(); }

// --- SendspinPersistenceProvider overrides ---

// THREAD CONTEXT: Main loop (invoked by client_->loop() during lifecycle events)
bool SendspinMcHub::save_last_server_hash(uint32_t hash) {
  LastPlayedServerPref pref{.server_id_hash = hash};
  bool ok = this->last_played_server_pref_.save(&pref);
  if (ok) {
    ESP_LOGD(TAG, "Persisted last played server hash: 0x%08" PRIX32, hash);
  } else {
    ESP_LOGW(TAG, "Failed to persist last played server hash");
  }
  return ok;
}

// THREAD CONTEXT: Main loop (invoked by client_->loop() during lifecycle events)
std::optional<uint32_t> SendspinMcHub::load_last_server_hash() {
  LastPlayedServerPref pref{};
  if (this->last_played_server_pref_.load(&pref)) {
    ESP_LOGI(TAG, "Loaded last played server hash: 0x%08" PRIX32, pref.server_id_hash);
    return pref.server_id_hash;
  }
  return std::nullopt;
}

// --- Sendspin role specific methods/overrides ---

#ifdef USE_SENDSPIN_MC_ARTWORK
// THREAD CONTEXT: Dedicated artwork decode thread; downstream callbacks run here too
void SendspinMcHub::on_image_decode(uint8_t slot, const uint8_t *data, size_t length,
                                  sendspin::SendspinImageFormat format) {
  this->artwork_image_decode_callbacks_.call(slot, data, length, format);
}

// THREAD CONTEXT: Main loop (fired from client_->loop() once the slot's offset-shifted display
// deadline is reached; lateness_ms reports how far past the deadline the display slipped)
void SendspinMcHub::on_image_display(uint8_t slot, uint32_t lateness_ms) {
  this->artwork_image_display_callbacks_.call(slot, lateness_ms);
}

// THREAD CONTEXT: Main loop (fired from client_->loop())
void SendspinMcHub::on_image_clear(uint8_t slot) { this->artwork_image_clear_callbacks_.call(slot); }

// THREAD CONTEXT: Main loop (invoked from SendspinMcImageSlot once a delivery is fully presented)
void SendspinMcHub::artwork_frame_done(uint8_t slot) {
  if (this->artwork_role_ != nullptr) {
    this->artwork_role_->frame_done(slot);
  }
}
#endif

#ifdef USE_SENDSPIN_MC_CONTROLLER
// THREAD CONTEXT: Main loop (invoked from ESPHome actions / other components)
void SendspinMcHub::send_client_command(sendspin::SendspinControllerCommand command, std::optional<uint8_t> volume,
                                      std::optional<bool> mute) {
  if (this->is_client_running() && this->controller_role_ != nullptr) {
    sendspin::ClientCommandControllerObject obj = {
        .command = command,
        .volume = volume,
        .muted = mute,
    };
    this->controller_role_->send_command(obj);
  }
}

// THREAD CONTEXT: Main loop (ControllerRoleListener override, fired from client_->loop())
void SendspinMcHub::on_controller_state(const sendspin::ServerStateControllerObject &state) {
  this->controller_state_callbacks_.call(state);
}

// THREAD CONTEXT: Main loop (ControllerRoleListener override, fired from client_->loop())
// Unlike metadata, this cannot be fanned out as a default-constructed state object: volume and muted are plain values
// rather than optionals, so children would read a real-looking 0% volume where we mean no value at all. A separate
// callback lets each child clear only what it can represent.
void SendspinMcHub::on_controller_state_clear() { this->controller_state_clear_callbacks_.call(); }
#endif

#ifdef USE_SENDSPIN_MC_METADATA
// THREAD CONTEXT: Main loop (MetadataRoleListener override, fired from client_->loop())
void SendspinMcHub::on_metadata(const sendspin::ServerMetadataStateObject &metadata) {
  this->metadata_update_callbacks_.call(metadata);
}

// THREAD CONTEXT: Main loop (MetadataRoleListener override, fired from client_->loop())
// The cached metadata was dropped because the connection to the server was lost, so what the children now mirror is
// the empty state. Fanning that out as a default-constructed state object rather than through a separate callback
// keeps one code path in the children: every field is nullopt, which they already publish as empty/unknown.
void SendspinMcHub::on_metadata_clear() { this->metadata_update_callbacks_.call(sendspin::ServerMetadataStateObject{}); }

// THREAD CONTEXT: Main loop (invoked from Sendspin components)
uint32_t SendspinMcHub::get_track_progress_ms() const {
  if (this->is_ready() && this->metadata_role_ != nullptr) {
    return this->metadata_role_->get_track_progress_ms();
  }
  return 0;
}
#endif

#ifdef USE_SENDSPIN_MC_PLAYER
// THREAD CONTEXT: Main loop, called from child component setup() after player role is created and configured
sendspin::PlayerRole *SendspinMcHub::get_player_role() {
  if (this->is_ready() && this->player_support_) {
    return this->client_->player();
  }
  return nullptr;
}

// THREAD CONTEXT: Main loop (SendspinPersistenceProvider override)
bool SendspinMcHub::save_static_delay(uint16_t delay_ms) {
  StaticDelayPref pref{.delay_ms = delay_ms};
  bool ok = this->static_delay_pref_.save(&pref);
  if (ok) {
    ESP_LOGD(TAG, "Persisted static delay: %u ms", delay_ms);
  } else {
    ESP_LOGW(TAG, "Failed to persist static delay");
  }
  return ok;
}

// THREAD CONTEXT: Main loop (SendspinPersistenceProvider override)
std::optional<uint16_t> SendspinMcHub::load_static_delay() {
  StaticDelayPref pref{};
  if (this->static_delay_pref_.load(&pref)) {
    ESP_LOGI(TAG, "Loaded static delay: %u ms", pref.delay_ms);
    return pref.delay_ms;
  }
  return std::nullopt;
}

#endif

}  // namespace esphome::sendspin_mc

#endif  // USE_ESP32
