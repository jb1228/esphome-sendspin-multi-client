...
static const char *const TAG = "sendspin_mc.image";
...
void SendspinMcImageSlot::setup() {
...
void SendspinMcImageSlot::on_decode_(const uint8_t *data, size_t length) {
  uint8_t *target;
  {
...
bool SendspinMcImageSlot::decode_frame_(const uint8_t *data, size_t length, const uint8_t *target) {
...
void SendspinMcImageSlot::on_display_(uint32_t lateness_ms) {
  bool frame_ready;
  {
    LockGuard lock(this->pending_mutex_);
    ...
    this->set_timeout("transition_ack", TRANSITION_ACK_WARNING_MS, [this]() {
      if (this->transition_pending_) {
        ESP_LOGW(TAG,
                 ...
                 "sendspin_mc.image.transition_finished runs or the stream is cleared",
                 this->slot_);
      }
    });
  }
...
void SendspinMcImageSlot::finish_transition_() {
...
void SendspinMcImageSlot::transition_finished() {
  if (!this->transition_pending_) {
    return;
  }
  this->finish_transition_();
...
void SendspinMcImageSlot::on_clear_() {
  {
    LockGuard lock(this->pending_mutex_);
...
// THREAD CONTEXT: Main loop.
void SendspinMcImageSlot::dump_config() {
  ESP_LOGCONFIG(TAG,
                "Artwork slot %u:\n"
                "  Dimensions: %dx%d\n"
...
// THREAD CONTEXT: Main loop.
void SendspinMcImageSlot::apply_frames_(bool transition_is_artwork) {
  this->current_image_->set_frame(this->buffers_[this->current_index_], this->width_, this->height_);
...
void SendspinMcImageSlot::report_error_() {
