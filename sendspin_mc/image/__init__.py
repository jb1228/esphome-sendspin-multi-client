...
SendspinMcImageSlot = sendspin_mc_ns.class_(
    "SendspinMcImageSlot",
    cg.Component,
    cg.Parented.template(SendspinMcHub),
)
ArtworkImageView = sendspin_mc_ns.class_("ArtworkImageView", Image_)
...
CONFIG_SCHEMA = cv.All(
    runtime_image.runtime_image_schema(SendspinMcImageSlot).extend(
        {
            cv.GenerateID(): cv.declare_id(SendspinMcImageSlot),
...
# TEMPORARY DEVIATION: Assign slots after parent IDs resolve, so mixed explicit/implicit
# references share one per-hub slot counter; see DEVIATIONS.md.
FINAL_VALIDATE_SCHEMA = _assign_slot_and_register
...
SendspinMcImageTransitionFinishedAction = sendspin_mc_ns.class_(
    "SendspinMcImageTransitionFinishedAction",
    automation.Action,
    cg.Parented.template(SendspinMcImageSlot),
)


@automation.register_action(
    "sendspin_mc.image.transition_finished",
    SendspinMcImageTransitionFinishedAction,
    automation.maybe_simple_id(
        cv.Schema(
            {
                cv.GenerateID(): cv.use_id(SendspinMcImageSlot),
            }
        )
    ),
    synchronous=True,
