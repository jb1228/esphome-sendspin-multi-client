...
/// @brief Decode-only RuntimeImage that decodes into a buffer owned by SendspinMcImageSlot.
///
...
/// @brief A non-owning image::Image view over a buffer owned by SendspinMcImageSlot.
///
...
class SendspinMcImageSlot : public SendspinMcChild {
 public:
  SendspinMcImageSlot(uint8_t slot, ArtworkImageView *current_image, int width, int height,
